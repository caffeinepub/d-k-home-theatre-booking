...
import { Calendar, Clock, Film, ChevronRight, Sparkles } from 'lucide-react';
...
const ScreeningCard = memo(function ScreeningCard({ screening, idx, onSelectScreening }: ScreeningCardProps) {
...
const ScreeningList = memo(function ScreeningList({ onSelectScreening }: ScreeningListProps) {
  ...
  return (
    <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
      {sortedScreenings.map((screening, idx) => (
        <ScreeningCard
          key={screening.id}
          screening={screening}
          idx={idx}
